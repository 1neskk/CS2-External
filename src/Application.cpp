...
bool Application::CreateDeviceD3D(HWND hWnd)
{
    ...
    if (D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, 0, featureLevelArray, 1,
        D3D11_SDK_VERSION, &sd, &s_pSwapChain, &s_pd3dDevice, &featureLevel, &s_pd3dDeviceContext) != S_OK)
    {
        return false;
    }
...
{
    CleanupRenderTarget();
    if (s_pSwapChain) { s_pSwapChain->Release(); s_pSwapChain = nullptr; }
    if (s_pd3dDeviceContext) { s_pd3dDeviceContext->Release(); s_pd3dDeviceContext = nullptr; }
...
{
    if (s_mainRenderTargetView) { s_mainRenderTargetView->Release(); s_mainRenderTargetView = nullptr; }
}
...
void Application::Init()
{
    ...
    RegisterClassExW(&wc);
    hwnd = CreateWindowExW(WS_EX_TOPMOST | WS_EX_TRANSPARENT | WS_EX_NOACTIVATE,
        wc.lpszClassName, L"ImGui Example", WS_POPUP, 0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN),
        ...
        ImFont* robotoRegular = io.Fonts->AddFontFromMemoryTTF((void*)g_RobotoRegular, sizeof(g_RobotoRegular), 16.0f, &config);
